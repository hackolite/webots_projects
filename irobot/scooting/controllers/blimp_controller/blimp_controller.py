...
# Poussee max pilote basse pour laisser de la place à la stabilisation
THRUST_H   = 3.0
THRUST_V   = 3.0
THRUST_YAW = 2.0
...
# Gains divisés pour stopper le tremblement / les oscillations
KP_ATT = 2.0      
KD_ATT = 0.6      
MAX_ATT_CORR = 4.0  
ATT_DEADBAND = 0.02 
...
while robot.step(timestep) != -1:
    ...
    # --------------------------------------------------------
    # 7. Commandes moteurs finales (Bridées STRICTEMENT sous 10)
    # --------------------------------------------------------
    base1 = (vx + vyaw) * MOTOR_SCALE_H
    base2 = (vx - vyaw) * MOTOR_SCALE_H

    omega1 = base1 - corr_pitch + corr_roll
    omega2 = base2 + corr_pitch + corr_roll

    # Limite informatique stricte à +/- 9.5 rad/s pour éviter les WARNING Webots
    ...
    if log_timer >= 1.0:
        ...
        pos_str = f"x={pos[0]:.1f} y={pos[1]:.1f} z={altitude:.1f}" if (gps and altitude) else "GPS N/A"
        alt_err = (target_altitude - altitude) if (target_altitude and altitude) else 0.0

        print(f"[NAV] {pos_str} | cmd={cmd_str}"
              f"| vx={vx:.2f} vz={vz:.2f} vyaw={vyaw:.3f}"
              f"| roll={math.degrees(roll):.1f}° pitch={math.degrees(pitch):.1f}°"
              f"| alt_err={alt_err:.2f}m corr_alt={corr_alt:.1f}")
